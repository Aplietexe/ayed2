 ...
 */
static unsigned int array_value_count(player_t a[],
                                      unsigned int length,
                                      player_t value) {
 ...
 */
static unsigned int process_string(FILE *file,
                                   char separator,
                                   char result[],
    ...
    while (!feof(file) && c != separator) {
        ...
        if (c != separator && str_size < result_max_size - 1u) {
            ...
            fprintf(
                stderr, "Max string length reached: %u \n", result_max_size);
            exit(EXIT_FAILURE);
        }
    }
    result[str_size] = '\0';
...
static unsigned int process_FILE(FILE *file, player_t atp[]) {
    ...
    while (!feof(file)) {
        ...
        process_string(
            file, SEPARATOR, atp[i].country, MAX_COUNTRY_LENGTH + 1u);
