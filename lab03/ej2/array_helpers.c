 ...
 */
static bool is_last_line(unsigned int year,
                         unsigned int month,
                         unsigned int day) {
    ...
    for (unsigned int year = 0u; year < YEARS; ++year) {
        for (month_t month = january; month <= december; ++month) {
            for (unsigned int day = 0u; day < DAYS; ++day) {
                fprintf(
                    stdout, "%u %u %u ", year + FST_YEAR, month + 1, day + 1);
...
static bool validate_date(unsigned year, unsigned month, unsigned day) {
    return FST_YEAR <= year && year <= LST_YEAR && FST_MONTH <= month &&
           month <= LST_MONTH && FST_DAY <= day && day <= LST_DAY;
}
...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {  // this is always bad
        ...
        if (!validate_date(k_year, k_month, k_day)) {
            fprintf(stderr, "Invalid date.\n");
            exit(EXIT_FAILURE);
        }
        array[k_year - FST_YEAR][k_month - FST_MONTH][k_day - FST_DAY] =
            weather;
    }
    fclose(file);
}
